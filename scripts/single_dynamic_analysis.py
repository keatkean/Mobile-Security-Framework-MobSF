...
def start_scan(file, server_url, apikey, rescan='0'):
    print('\nLooking for Android/iOS/Windows binaries or source code: ' + file)
    ...
    if ext in mimes:
        ...
        if response.status_code == 200 and 'hash' in response.json():
            logging.info('[OK] Upload Complete')
            ...
            if response.status_code == 200:
                logging.info('[OK] Static Analysis Complete')
            else:
                logging.error('[Error] Performing Static Analysis')
        else:
            logging.error('[Error] Performing Upload')
        ...
        logging.error("[Error] Please upload APK/IPA/APPX/ZIP files only")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file', help='Path to the file that contains mobile app binary/zipped source code')
    ...
    if args.file and args.ipport and args.apikey:
        server = args.ipport
        file = args.file
        ...
        start_scan(file, server_url, apikey, rescan)
    else:
        parser.print_help()